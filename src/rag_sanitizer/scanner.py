...
from .detectors.entity_swap import detect_entity_swap
from .detectors.mimicry import detect_mimicry
from .detectors.multimodal import detect_multimodal
...
def _combine(mimicry_flag: bool, entity_flag: bool, multimodal_flag: bool) -> tuple[str, list[str]]:
    reasons: list[str] = []
    if mimicry_flag:
    ...
    if multimodal_flag:
        reasons.append("multimodal structural mismatch (possible visual poison, v0.1 heuristic)")
    if not reasons:
        return "CLEAN", reasons
    # POISON if mimicry OR entity-swap (content-level); SUSPECT if only multimodal
    # heuristic (conservative, KI-2).
...
def scan(
    ...
    profile (declared unsupervised mode). Deterministic (AC-5): no unseeded RNG.
    """
    clean = [d for d in docs if d.trust == "clean"]
    if not clean:
        # Unsupervised: trust the documents closest to the global centroid.
        clean = list(docs)
    ...
    for d in docs:
        ...
        ent = detect_entity_swap(d.text, profile_texts)
        mm = detect_multimodal(d.text, d.image_path)
        verdict, reasons = _combine(mim.flagged, ent.flagged, mm.flagged)
        if mm.flagged and mm.reason:
            reasons.append(mm.reason)
        verdicts.append(DocVerdict(id=d.id, verdict=verdict, reasons=reasons))

    counts = {"CLEAN": 0, "SUSPECT": 0, "POISON": 0}
    for v in verdicts:
        counts[v.verdict] += 1
    return ScanReport(corpus_sha256=corpus_sha256(docs), verdicts=verdicts, summary=counts)
